...
- **Node.js 服务**：VarifyServer 使用 Node.js、@grpc/grpc-js、@grpc/proto-loader、amqplib、ioredis、nodemailer、pino、uuid。
...
- **本地基础设施**：Docker、Docker Compose、Nginx、nginx-prometheus-exporter、Redis Stack、Postgres、MongoDB、MinIO、Redpanda、RabbitMQ、Neo4j、Qdrant、AIOrchestrator Docker 服务。
- **可观测性**：Prometheus、Alertmanager、Grafana、Loki、Tempo、OpenTelemetry Collector、OpenTelemetry SDK/OTLP、InfluxDB、cAdvisor、nginx exporter、LangSmith tracing、Prometheus Python client。
- **运维与部署**：MemoOps QML、OpsCollector Python、Nginx local gateway、Docker bind data、Kubernetes manifests、Helm chart、Kustomize overlays、etcd 配置/服务发现、可选 ELK compatibility stack（Elasticsearch、Logstash、Kibana、Filebeat）。
...
- **服务端口**：GPT-SoVITS API `9880`、local vision inference sidecar `8097`、realtime bridge sidecar `8098`。这些端口需要评审后再启用，不能静默改变现有固定端口。
