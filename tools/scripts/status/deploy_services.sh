...
BUILD_BIN="${MEMOCHAT_BUILD_BIN:-${PROJECT_ROOT}/build-linux-server-gcc16/bin}"
...
usage() {
    cat <<USAGE
Usage: $0 [--build-bin PATH] [--runtime-dir PATH] [--check-only]
...
copy_optional "${BUILD_BIN}/MemoChatQml" "${RUNTIME_DIR}/MemoChatQml/MemoChatQml"
copy_optional "${BUILD_BIN}/config.ini" "${RUNTIME_DIR}/MemoChatQml/config.ini"
