...
# ordered endpoint lists when an experimental transport cannot connect.
...
launch_svc() {
    ...
    launch_detached_service "$svc_dir" "$exe_name" "$out_log" "$err_log" "$pid_file" \
        ...
        MEMOCHAT_ADVERTISE_WEBTRANSPORT="${MEMOCHAT_ADVERTISE_WEBTRANSPORT:-1}" \
        MEMOCHAT_ENABLE_LWS_WEBTRANSPORT_PROVIDER="${MEMOCHAT_ENABLE_LWS_WEBTRANSPORT_PROVIDER:-1}" \
