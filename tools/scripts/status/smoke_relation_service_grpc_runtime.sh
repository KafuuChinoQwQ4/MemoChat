...
TEST_BIN="${MEMOCHAT_RELATION_SERVICE_SMOKE_BIN:-${PROJECT_ROOT}/build-linux-full-gcc16/tests/chatserver/chatserver_relation_grpc_client_gtest}"
LOCAL_COMPOSE_FILE="${MEMOCHAT_LOCAL_COMPOSE_FILE:-${PROJECT_ROOT}/infra/deploy/local/docker-compose.yml}"
RELATION_SERVICE_ENDPOINT="${MEMOCHAT_RELATION_SERVICE_SMOKE_ENDPOINT:-127.0.0.1:50091}"
CHATSERVER_TCP_PORT="${MEMOCHAT_RELATION_SERVICE_SMOKE_CHAT_TCP_PORT:-18091}"
CHATSERVER_QUIC_PORT="${MEMOCHAT_RELATION_SERVICE_SMOKE_CHAT_QUIC_PORT:-18191}"
CHATSERVER_RPC_PORT="${MEMOCHAT_RELATION_SERVICE_SMOKE_CHAT_RPC_PORT:-15056}"
WAIT_SECONDS="${MEMOCHAT_RELATION_SERVICE_SMOKE_WAIT_SECONDS:-20}"
KEEP_RUNTIME=0
TMP_ROOT=""
RUNTIME_DIR=""
LOG_DIR=""
...
Run an opt-in runtime smoke for:
  ChatServer temp config -> RelationService Backend=grpc -> ChatRelationServiceWorker

Required built artifacts:
  ${BUILD_BIN}/ChatRelationServiceWorker
...
wait_for_storage_deps() {
    ...
    while (( waited < WAIT_SECONDS )); do
        if docker exec memochat-redis redis-cli -a 123456 ping >/dev/null 2>&1 &&
            ...
            echo "  [OK] Redis/Postgres ready for relation service smoke"
            return 0
        fi
        sleep 1
        waited=$((waited + 1))
    done
    echo "[FAIL] Redis/Postgres did not become ready for relation service smoke" >&2
    return 1
}

temp_runtime_processes() {
    local exe pid cwd
    for exe in ChatServer ChatRelationServiceWorker; do
...
if [[ "$RELATION_SERVICE_ENDPOINT" != "127.0.0.1:50091" ]]; then
    echo "[FAIL] This smoke currently expects MEMOCHAT_RELATION_SERVICE_SMOKE_ENDPOINT=127.0.0.1:50091" >&2
    exit 1
fi

require_executable "${BUILD_BIN}/ChatRelationServiceWorker"
require_executable "${BUILD_BIN}/ChatServer"
require_executable "$TEST_BIN"

TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/memochat-relation-service-grpc-smoke.XXXXXX")"
...
relation_config="${RUNTIME_DIR}/ChatRelationServiceWorker1/config.ini"
...
set_ini_value "$chat_config" "RelationService" "Backend" "grpc"
set_ini_value "$chat_config" "RelationService" "Endpoint" "127.0.0.1:50091"
...
set_ini_value "$relation_config" "Log" "Dir" "${LOG_DIR}/chatrelationservice1-runtime"

grep -q '^Backend=grpc$' "$chat_config"
grep -q '^Endpoint=127.0.0.1:50091$' "$chat_config"

echo "============================================================"
echo "  RelationService gRPC runtime smoke"
echo "  TMP_ROOT:    ${TMP_ROOT}"
echo "  RUNTIME_DIR: ${RUNTIME_DIR}"
echo "  CONFIG:      ${chat_config}"
echo "============================================================"

if tcp_port_listening "50091"; then
    echo "[FAIL] Relation service smoke port 50091 is already in use" >&2
    exit 1
fi
...
chat_out="${LOG_DIR}/ChatServer-remote-relation-service_out.log"
chat_err="${LOG_DIR}/ChatServer-remote-relation-service_err.log"
chat_pid_file="${PID_DIR}/ChatServer-1.pid"
: > "$chat_out"
: > "$chat_err"

echo "[STEP] Start ChatServer with relation service grpc backend"
(
...
probe_log="${LOG_DIR}/relation_service_remote_probe.log"
echo "[STEP] Run RelationGrpcClient command probe against real ChatRelationServiceWorker"
MEMOCHAT_RELATION_SERVICE_SMOKE_ENDPOINT="$RELATION_SERVICE_ENDPOINT" \
    "$TEST_BIN" --gtest_filter='RelationGrpcClientTest.RuntimeSmokeSearchUserUsesRealRelationServiceWorker' |
    tee "$probe_log"

if grep -Fq "relation_remote_error" "$probe_log"; then
    echo "[FAIL] Probe reported relation_remote_error" >&2
    exit 1
fi
...
echo "[SUCCESS] RelationService gRPC runtime smoke passed"
