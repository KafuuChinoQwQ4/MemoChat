...
function handleCookieChange(changeInfo) {
    if (!activeTransaction || activeTransaction.status !== 'waiting')
        return;
    const cookie = changeInfo.cookie;
    if (!cookie || changeInfo.removed)
        return;
    // Check if this is an allowed E-Hentai cookie
    if (!ALLOWED_COOKIE_NAMES.has(cookie.name))
        return;
    if (!EHENTAI_DOMAINS.some(domain => cookie.domain.includes(domain)))
        return;
    // Store the cookie value (never log it)
    activeTransaction.collectedCookies[cookie.name] = cookie.value;
