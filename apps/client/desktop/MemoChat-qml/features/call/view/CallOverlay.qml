...
Item {
    ...
    function forwardBridgeEvent(encodedTitle) {
        ...
        try {
            ...
            root.livekitBridge.reportMediaError("LiveKit 页面事件解析失败")
            return
        }

        const type = payload.type || ""
        if (type === "joined") {
            root.livekitBridge.reportRoomJoined()
        } else if (type === "remote_track_ready") {
            root.livekitBridge.reportRemoteTrackReady()
        } else if (type === "disconnected") {
            root.livekitBridge.reportRoomDisconnected(payload.reason || "", payload.recoverable === true)
        } else if (type === "permission_error") {
            root.livekitBridge.reportPermissionError(payload.deviceType || "", payload.message || "")
        } else if (type === "media_error") {
            root.livekitBridge.reportMediaError(payload.message || "")
        } else if (type === "reconnecting") {
            root.livekitBridge.reportReconnecting(payload.message || "")
        } else if (type === "log") {
            root.livekitBridge.reportLog(payload.message || "")
        }
    }
    ...
    WebEngineView {
        id: mediaView
        anchors.fill: parent
        visible: root.mediaState && root.livekitBridge && root.livekitBridge.embeddedEnabled
        url: root.livekitBridge ? root.livekitBridge.roomPageUrl : ""
        ...
        onLoadingChanged: function(loadRequest) {
            if (loadRequest.status === root.loadSucceededStatus) {
                ...
                root.livekitBridge.reportMediaError("LiveKit 页面加载失败")
            }
        }
