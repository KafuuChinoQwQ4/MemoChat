...
QString displayTextForSpeech(const QString& speechText)
{
...
bool isObservationControlEvent(const QJsonObject& event)
{
    ...
    return actionName == QStringLiteral("observe") || actionName == QStringLiteral("visual_react");
}
...
void PetModel::applyControlEvent(const QJsonObject& event)
{
    ...
    const bool preserve_voice_for_observation =
        isObservationControlEvent(event) && !_audio_url.isEmpty() && isAudioStateStillPresenting(_audio_state);

    const QString next_phase =
        preserve_voice_for_observation ? _phase : jsonString(event, QStringLiteral("phase"), _phase);
    ...
    if (!preserve_voice_for_observation)
    {
        ...
        const QString next_expression =
            jsonString(animation,
                       QStringLiteral("expression"), jsonString(event, QStringLiteral("expression"), _expression));
        if (updateString(_expression, next_expression))
        {
            changed = true;
        }

        const QString next_motion =
            jsonString(animation,
                       QStringLiteral("motion"),
                                      jsonString(animation,
                                                 QStringLiteral("motion_group"),
                                                                jsonString(event, QStringLiteral("motion"), _motion)));
    ...
    if (!preserve_voice_for_observation)
    {
        const qreal next_lip =
            boundedUnit(jsonReal(lip_sync,
                                 QStringLiteral("value"), jsonReal(audio, QStringLiteral("rms"), _lip_sync_value)),
                                 _lip_sync_value);
    ...
    const QString delta =
        jsonString(text, QStringLiteral("delta"), jsonString(speech, QStringLiteral("text_delta"), QString()));
    ...
    const bool has_text_update =
        !preserve_voice_for_observation &&
        (!delta.isEmpty() || text_final || !display_text.isEmpty() || !translated_text.isEmpty());
    ...
    if (reset_speech_for_waiting_phase && !preserve_voice_for_observation &&
    ...
    const QString next_display_text = displayTextForSpeech(_speech_text);
...
void PetModel::clearSpeech()
{
    if (_speech_text.isEmpty() && _speech_translation.isEmpty() && _speech_display_text.isEmpty() && !_speech_final &&
        _audio_url.isEmpty() && _audio_state == QStringLiteral("idle"))
    {
        return;
    }
    _speech_text.clear();
