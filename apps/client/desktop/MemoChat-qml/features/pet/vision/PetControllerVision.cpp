                                       ...
                                       int frameHeight)
{
    ...
    postVisionCapture(encodedFrame,
                      ...
                      frameWidth,
                      frameHeight,
                      QStringLiteral("qt_camera"), QStringLiteral("local_frame_upload"));
}
...
bool PetController::captureVisionVideoFrame(const QVideoFrame& frame, int frameWidth, int frameHeight)
{
    ...
    const EncodedVisionFrame encoded = encodeVisionVideoFrameAsJpeg(frame, 82);
    if (!encoded.ok)
    {
    ...
    return postVisionCapture(encoded.frameBase64,
                             encoded.frameMime,
                             frameWidth,
                             frameHeight,
                             QStringLiteral("qt_video_sink"), QStringLiteral("live_frame_upload"));
}
...
QString PetController::captureVisionSegmentVideoFrame(const QVideoFrame& frame, int frameWidth, int frameHeight)
{
    ...
    const EncodedVisionFrame encoded = encodeVisionVideoFrameAsJpeg(frame, 72);
    if (!encoded.ok)
    {
        if (!encoded.error.isEmpty())
        {
    ...
    if (shouldSkipVisionFrame(encoded.signature, now, QStringLiteral("qt_video_sink_segment")))
    {
        return QStringLiteral("视觉画面未变化，已跳过");
    }

    return appendVisionSegmentFrame(encoded.frameBase64,
                                    encoded.frameMime,
                                    frameWidth,
                                    frameHeight,
                                    encoded.signature,
                                    QStringLiteral("qt_video_sink_segment"), QStringLiteral("keyframe_segment_upload"));
}
#endif
                                      ...
                                      const QString& transport)
{
    ...
    metadata[QStringLiteral("source")] = source.trimmed().isEmpty() ? QStringLiteral("qt_camera") : source.trimmed();
    metadata[QStringLiteral("transport")] = transport.trimmed().isEmpty()                         ? QStringLiteral("local_frame_upload")
                                                                                                  : transport.trimmed();
    ...
    payload[QStringLiteral("frame_mime")] = frameMime.trimmed().isEmpty() ? QStringLiteral("image/jpeg")
                                                                          : frameMime.trimmed();
                                                ...
                                                const QString& transport)
{
    ...
    frame[QStringLiteral("frame_mime")] = frameMime.trimmed().isEmpty() ? QStringLiteral("image/jpeg")
                                                                        : frameMime.trimmed();
    ...
    _vision_segment_started_at_ms = _vision_segment_frames.isEmpty()
                                        ? 0
                                        : _vision_segment_frames.first()
                                              .toObject()
                                              .value(QStringLiteral("captured_at_ms")).toVariant().toLongLong();
    ...
    return QStringLiteral("视觉环形缓冲采样中 %1/%2") .arg(_vision_segment_frames.size()).arg(kVisionSegmentMaxFrames);
}
...
void PetController::postVisionSegment(const QString& source, const QString& transport)
{
    ...
    QJsonObject metadata;
    metadata[QStringLiteral("source")] = source.trimmed().isEmpty()                  ? QStringLiteral("qt_video_sink_segment")
                                                                                     : source.trimmed();
    metadata[QStringLiteral("transport")] = transport.trimmed().isEmpty()            ? QStringLiteral("keyframe_segment_upload")
                                                                                     : transport.trimmed();
    metadata[QStringLiteral("reply_language")] = _reply_language.trimmed().isEmpty() ? QStringLiteral("zh-CN")
                                                                                     : _reply_language.trimmed();
    const QString speechRules = _speech_rules.trimmed();
...
void PetController::captureVisionFrameFileWithMetadata(const QString& filePath,
                                                       ...
                                                       const QString& transport)
{
    const QString localPath = filePath.trimmed().startsWith(QStringLiteral("file://"))
                                                            ? QUrl(filePath.trimmed()).toLocalFile()
                                                            : filePath.trimmed();
    ...
    const EncodedVisionFrame encoded = readVisionFrameFile(localPath);
    QFile::remove(localPath);
    if (!encoded.ok)
    {
        if (!encoded.error.isEmpty())
    ...
    postVisionCapture(encoded.frameBase64, encoded.frameMime, frameWidth, frameHeight, source, transport);
}
