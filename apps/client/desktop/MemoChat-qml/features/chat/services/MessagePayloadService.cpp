...
{
    if (deletedAtMs > 0 || msgType == QStringLiteral("revoke") || content == QStringLiteral("[消息已撤回]"))
    {
        ...
        obj.value(QStringLiteral("msgid")).toString(),
                  ...
                  obj.value(QStringLiteral("fromuid")).toInt(),
                            obj.value(QStringLiteral("touid")).toInt(),
                                      QString(),
                                      normalizeEpochMs(
                                          obj.value(QStringLiteral("created_at")).toVariant().toLongLong(), true),
                                          QString(),
...
MessagePayloadService::buildPrivateForwardedMessage(const QJsonObject& payload, const QJsonObject& msgObj, int fromUid)
{
                                                                          ...
                                                                          const QString& fromIcon)
{
    ...
    return std::make_shared<TextChatData>(
        ...
        createdAt,
        fromIcon,
        resolveMessageState(content, editedAtMs, deletedAtMs, msgObj.value(QStringLiteral("msgtype")).toString()),
                            serverMsgId,
                            groupSeq,
                            replyToServerMsgId,
                            forwardMetaJson,
