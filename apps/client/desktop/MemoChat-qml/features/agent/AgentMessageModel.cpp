...
AssistantParts splitAssistantParts(const QString& raw)
{
    ...
    QRegularExpression openBlock(QStringLiteral("<think>([\\s\\S]*)$"), QRegularExpression::CaseInsensitiveOption);
...
QVariant AgentMessageModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role)
    {
        case MsgIdRole:
            return entry.msgId;
        case ContentRole:
            return entry.isStreaming ? entry.streamingContent : entry.content;
        case RoleRole:
            return entry.role;
        case CreatedAtRole:
        ...
        case StreamingContentRole:
            return entry.streamingContent;
        case ThinkingContentRole:
            return entry.thinkingContent;
        case TokensUsedRole:
...
{
    MessageEntry entry;
    entry.msgId = msgId.isEmpty() ? QString("ai_") + QString::number(QDateTime::currentMSecsSinceEpoch()) : msgId;
    entry.content = "";
