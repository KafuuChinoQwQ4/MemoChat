...
void AppController::saveProfile(const QString& nick, const QString& desc)
{
    ...
    QString iconForSave = _user_state.icon;
    const QUrl iconUrl(iconForSave);
    if (iconUrl.isLocalFile() || QFileInfo(iconForSave).isAbsolute())
    {
        if (_media_upload_state.settingsAvatarUploadInProgress)
        {
        ...
        _media_upload_state.settingsAvatarUploadInProgress = true;
        ...
        connect(watcher,
                &QFutureWatcher<ProfileAvatarUploadResult>::finished,
                this,
        ...
        const auto future = QtConcurrent::run(
            [uploadPath, selfUid, uploadToken]()
            {
    ...
    _profile_controller.sendSaveProfile(selfUid, selfName, nick, desc, iconForSave);
    setSettingsStatus("资料同步中...", false);
}
