...
void AppController::startCallFlow(const QString& callType)
{
    ...
    _call_controller.startCall(selfInfo->_uid, _gateway.userMgr()->GetToken(), _contact_state.uid, callType);
...
void AppController::onCallHttpFinished(ReqId id, QString res, ErrorCodes err)
{
    ...
    {
        _call_session_model.startOutgoing(obj.value("call_id").toString(),
                                          obj.value("call_type").toString(),
                                          obj.value("peer_name").toString(_contact_state.name),
                                          normalizeIconPath(obj.value("peer_icon").toString(_contact_state.icon)),
    ...
    if (id == ReqId::ID_CALL_GET_TOKEN)
    {
        ...
        _livekit_bridge.requestJoinRoom(livekitUrl,
                                        token,
...
void AppController::onCallEvent(QJsonObject payload)
{
    ...
    const QString peerIcon =
        normalizeIconPath(isCaller ? payload.value("callee_icon").toString() : payload.value("caller_icon").toString());

    if (eventType == QStringLiteral("call.invite") && calleeUid == selfUid)
    {
        _call_session_model.startIncoming(callId,
                                          callType,
                                          peerName,
                                          peerIcon,
                                          QStringLiteral("收到来电"),
    ...
    if (eventType == QStringLiteral("call.accepted"))
    {
        ...
        {
            _call_session_model.startOutgoing(callId,
                                              callType,
                                              peerName,
                                              peerIcon,
                                              QStringLiteral("已接通"),
