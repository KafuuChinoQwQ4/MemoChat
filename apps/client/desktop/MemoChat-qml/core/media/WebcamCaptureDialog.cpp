...
private slots:
...
#else
        QMessageBox::information(this, QStringLiteral("提示"), QStringLiteral("摄像头功能未启用"));
#endif
    }
    ...
    void initCamera()
    {
        ...
        for (const QCameraInfo& ci : cameras)
        {
            ...
            if (ci.deviceName().contains(QStringLiteral("back"), Qt::CaseInsensitive) ||
                                         ci.deviceName().contains(QStringLiteral("内置"), Qt::CaseInsensitive))
            {
                cameraInfo = ci;
                break;
            }
        ...
        connect(_imageCapture, &QCameraImageCapture::imageCaptured, this, &WebcamCaptureDialogPrivate::onImageCaptured);
        connect(_imageCapture,
                &QCameraImageCapture::error,
                this,
                [this](int, QCameraImageCapture::Error, const QString& err)
                {
