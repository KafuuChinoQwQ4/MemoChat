...
void ScreenCaptureWidget::paintEvent(QPaintEvent*)
{
    ...
    if (_selecting && _selectionRect.isValid())
    {
        ...
        QList<QRect> handles = {
            QRect(_selectionRect.topLeft().x() - handleSize / 2,
                  _selectionRect.topLeft().y() - handleSize / 2,
                  handleSize,
                  handleSize),
            QRect(_selectionRect.topRight().x() - handleSize / 2,
                  _selectionRect.topRight().y() - handleSize / 2,
                  handleSize,
                  handleSize),
            QRect(_selectionRect.bottomLeft().x() - handleSize / 2,
                  _selectionRect.bottomLeft().y() - handleSize / 2,
                  handleSize,
                  handleSize),
            QRect(_selectionRect.bottomRight().x() - handleSize / 2,
                  _selectionRect.bottomRight().y() - handleSize / 2,
                  handleSize,
                  handleSize),
        };
        ...
        if (_selectionRect.width() > 60 && _selectionRect.height() > 30)
        {
            const QString sizeText =
                QString::number(_selectionRect.width()) + " x " + QString::number(_selectionRect.height());
            QRect textBg = QRect(_selectionRect.center().x() - 40, _selectionRect.bottom() + 4, 80, 18);
...
{
    const QPoint topLeft(std::min(_startPos.x(), _currentPos.x()), std::min(_startPos.y(), _currentPos.y()));
    const QPoint bottomRight(std::max(_startPos.x(), _currentPos.x()), std::max(_startPos.y(), _currentPos.y()));
