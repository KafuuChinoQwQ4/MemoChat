...
void QuicChatTransport::connectToServer(ServerInfo serverInfo)
{
    ...
    const QUIC_STATUS status =
        _api->ConnectionOpen(_registration,
    ...
    const QUIC_STATUS startStatus =
        _api->ConnectionStart(_connection, _configuration, QUIC_ADDRESS_FAMILY_UNSPEC, hostUtf8.constData(), _port);
    ...
    qInfo() << "quic connect start. host:" << _host << "port:" << _port << "server:" << _serverName
...
void QuicChatTransport::slot_send_data(ReqId reqId, QByteArray data)
{
    ...
    ctx->bytes = encodeChatFrame(reqId, data);
    ctx->buffer.Buffer = reinterpret_cast<uint8_t*>(ctx->bytes.data());
    ctx->buffer.Length = static_cast<uint32_t>(ctx->bytes.size());

    const QUIC_STATUS status = _api->StreamSend(_stream, &ctx->buffer, 1, QUIC_SEND_FLAG_NONE, ctx);
    if (QUIC_FAILED(status))
    {
...
bool QuicChatTransport::ensureQuicReady(QString* errorText)
{
    ...
    status =
        _api->ConfigurationOpen(_registration, &alpnBuffer, 1, &settings, sizeof(settings), nullptr, &_configuration);
    if (QUIC_FAILED(status))
    {
        if (errorText)
        {
...
int QuicChatTransport::onConnectionEvent(void* eventPtr)
{
    ...
    switch (event->Type)
    {
        ...
        {
            const QUIC_STATUS openStatus =
                _api->StreamOpen(_connection,
