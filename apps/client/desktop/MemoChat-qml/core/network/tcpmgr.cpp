    ...
    , _connecting(false)
{
    ...
    QObject::connect(&_connect_timeout_timer,
                     &QTimer::timeout,
                     [this]()
                     {
                         if (!_connecting)
    ...
    QObject::connect(&_socket,
                     ...
                     [this]()
                     {
                         const QVector<ChatFrame> frames = _frame_parser.append(_socket.readAll());
                         for (const ChatFrame& frame : frames)
                         {
                             handleMsg(frame.reqId, frame.length, frame.payload);
                         }
                     });
    ...
    QObject::connect(&_socket,
                     ...
                     [this](QAbstractSocket::SocketError socketError)
                     {
                         qWarning() << "tcp socket error:" << socketErrorName(socketError)
                                    << "message:" << _socket.errorString() << "host:" << _host << "port:" << _port;
...
{
    qInfo() << "closing tcp connection. host:" << _host << "port:" << _port << "state:" << _socket.state();
    _connect_timeout_timer.stop();
    _connecting = false;
    _socket.close();
}
...
void TcpMgr::connectToServer(ServerInfo si)
{
    ...
    qInfo() << "connecting to chat server, host:" << _host << "port:" << _port
            << "timeout_ms:" << _connect_timeout_timer.interval() << "protocol_version:" << si.ProtocolVersion;
...
void TcpMgr::slot_send_data(ReqId reqId, QByteArray dataBytes)
{
    ...
    const QByteArray block = encodeChatFrame(reqId, dataBytes);
