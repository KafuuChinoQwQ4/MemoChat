...
{
    return qRgba(colorByte(color.red),
                 colorByte(color.green),
                 colorByte(color.blue),
                 colorByte(color.alpha));
}
...
StraightColor blendSourceOver(const StraightColor &source, const StraightColor &destination)
{
    ...
    result.green = (source.green * source.alpha + destination.green * destination.alpha * inverseSourceAlpha) / result.alpha;
    result.blue = (source.blue * source.alpha + destination.blue * destination.alpha * inverseSourceAlpha) / result.alpha;
    return result;
}
...
StraightColor blendMultiplicative(const StraightColor &source, const StraightColor &destination)
{
    ...
    result.red = qBound<qreal>(0.0, source.red * source.alpha * destination.red + destination.red * inverseSourceAlpha, 1.0);
    result.green = qBound<qreal>(0.0, source.green * source.alpha * destination.green + destination.green * inverseSourceAlpha, 1.0);
    result.blue = qBound<qreal>(0.0, source.blue * source.alpha * destination.blue + destination.blue * inverseSourceAlpha, 1.0);
    return result;
}
...
struct Live2DCoreRenderer::CoreApi
{
    ...
    bool load()
    {
        ...
        for (const QString &candidate : coreLibraryCandidates()) {
            ...
            loaded = reviveMocInPlace && getSizeofModel && initializeModelInPlace && updateModel
                     && readCanvasInfo && getParameterCount && getParameterIds
                     && getParameterMinimumValues && getParameterMaximumValues
                     && getParameterDefaultValues && getParameterValues && getDrawableCount
                     && getRenderOrders && getDrawableConstantFlags && getDrawableDynamicFlags
                     && getDrawableBlendModes && getDrawableMaskCounts && getDrawableMasks && getDrawableTextureIndices
                     && getDrawableOpacities && getDrawableVertexCounts && getDrawableVertexPositions
                     && getDrawableVertexUvs && getDrawableIndexCounts && getDrawableIndices;
            if (loaded) {
...
{
    return loadModelFile(clientSourcePath(QStringLiteral("src/KafuuChino/香风智乃live2D/香风智乃.model3.json")));
}
...
void Live2DCoreRenderer::setParameter(const QString &id, float value, float weight)
{
    ...
    values[index] = clamped(blended(values[index], value, weight),
                            _parameterMinimums[index],
                            _parameterMaximums[index]);
}
...
void Live2DCoreRenderer::paintModel(QPainter *painter, const QRectF &stage)
{
    ...
    std::stable_sort(drawables.begin(), drawables.end(), [renderOrders](int lhs, int rhs) {
    ...
    for (int drawable : drawables) {
        ...
        if (maskCount > 0 && masks && masks[drawable]) {
            ...
            for (int maskSlot = 0; maskSlot < maskCount; ++maskSlot) {
                ...
                for (int i = 0; i + 2 < maskIndexCount; i += 3) {
                    ...
                    if (i0 < 0 || i1 < 0 || i2 < 0
                        || i0 >= maskVertexCount || i1 >= maskVertexCount || i2 >= maskVertexCount) {
                        continue;
                    }
                    ...
                    const QPointF t0(maskVertexUvs[i0].X * maskTexture.width(), (1.0f - maskVertexUvs[i0].Y) * maskTexture.height());
                    const QPointF t1(maskVertexUvs[i1].X * maskTexture.width(), (1.0f - maskVertexUvs[i1].Y) * maskTexture.height());
                    const QPointF t2(maskVertexUvs[i2].X * maskTexture.width(), (1.0f - maskVertexUvs[i2].Y) * maskTexture.height());
        ...
        for (int i = 0; i + 2 < indexCount; i += 3) {
            ...
            paintTexturedTriangle(layer, texture, p0, p1, p2, t0, t1, t2,
                                  opacity, blendMode, transform, clipMaskPtr, invertedMask);
        }
    }
