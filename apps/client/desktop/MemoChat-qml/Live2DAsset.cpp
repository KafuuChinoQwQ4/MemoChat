...
QString findSiblingVtubeMapping(const QString &modelDirectory)
{
    ...
    const QDir dir(modelDirectory);
    const QFileInfoList files = dir.entryInfoList({QStringLiteral("*.vtube.json")},
...
QString computePackageChecksum(const QString &modelDirectory, const QStringList &packageFiles)
{
    ...
    std::sort(files.begin(), files.end(), [&modelDirectory](const QString &left, const QString &right) {
...
void appendMissingDirectoryWarning(QStringList &warnings, const QString &label, const QString &input, const QString &resolvedPath)
{
...
void Live2DAsset::validate()
{
    ...
    if (cleanedInput(_model_json).isEmpty()) {
        publishResult(QString::fromLatin1(kStatusEmpty),
                      ...
                      expressionCount,
                      textureCount,
                      countFilesWithSuffixes(resolveInputPath(_voice_directory),
                                             {QStringLiteral(".wav"),
                                              QStringLiteral(".mp3"),
                                              QStringLiteral(".ogg"),
                                              QStringLiteral(".flac"),
    ...
    if (!fileExists(modelJsonPath) || QFileInfo(modelJsonPath).isDir()) {
        ...
        publishResult(QString::fromLatin1(kStatusMissing),
                      errors.first(),
                      errors,
                      warnings,
                      motionCount,
    ...
    if (!modelFile.open(QIODevice::ReadOnly)) {
        ...
        publishResult(QString::fromLatin1(kStatusInvalid),
                      errors.first(),
                      errors,
                      warnings,
                      motionCount,
    ...
    expressionCount = qMax(expressionCount,
                           countFilesWithSuffixes(resolvedExpressionDirectory,
                                                  {QStringLiteral(".exp3.json")}));
    const int voiceCount = countFilesWithSuffixes(resolvedVoiceDirectory,
                                                  {QStringLiteral(".wav"),
                                                   QStringLiteral(".mp3"),
                                                   QStringLiteral(".ogg"),
                                                   QStringLiteral(".flac"),
    ...
    if (!resolvedVoiceDirectory.isEmpty() && !isQrcPath(resolvedVoiceDirectory)) {
        ...
        const QFileInfoList voiceFiles = voiceDir.entryInfoList({QStringLiteral("*.wav"),
                                                                 QStringLiteral("*.mp3"),
                                                                 QStringLiteral("*.ogg"),
                                                                 QStringLiteral("*.flac"),
                                                                 QStringLiteral("*.m4a")},
    ...
    const QString statusText = errors.isEmpty()
                                   ? QStringLiteral("资源校验通过：%1 动作 / %2 表情 / %3 贴图 / %4 语音")
                                         .arg(motionCount)
                                         .arg(expressionCount)
                                         .arg(textureCount)
                                         .arg(voiceCount)
