...
function looksLikeMediaKey(ref) {
...
function mediaBaseUrl() {
    const configured = `${runtimeConfig.mediaBaseUrl ?? ""}`.trim() ||
        `${runtimeConfig.gateBaseUrl ?? ""}`.trim();
    return configured.replace(/\/+$/, "");
}
function urlParseBase() {
    return mediaBaseUrl() || "http://memochat.local";
}
function shouldReturnAbsolute(originalUrl) {
    return isHttpUrl(originalUrl) || Boolean(mediaBaseUrl());
}
function serializeUrl(parsed, absolute) {
    if (absolute)
...
export function resolveMediaUrl(ref) {
    ...
    if (!clean || isPassthroughUrl(clean))
        return clean;
    if (isDefaultAvatarRef(clean))
