...
describe("ClientGateway", () => {
    ...
    it("routes chat transport frames into the dispatcher", () => {
        ...
        gateway.chatTransport.onMessage?.(ReqId.ID_GET_RELATION_BOOTSTRAP_RSP, JSON.stringify({ error: 0, friend_list: [{ uid: 910002 }] }));
    ...
    it("keeps chat route handlers registered after React StrictMode remounts AppShell effects", () => {
        render(createElement(StrictMode, null, createElement(GatewayProvider, null, createElement(MemoryRouter, null, createElement(AppShell)))));
