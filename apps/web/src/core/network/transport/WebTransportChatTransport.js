...
export function getWebTransportConstructor() {
    return globalThis
        .WebTransport;
}
export class WebTransportChatTransport {
    constructor() {
    ...
    send(reqId, payload) {
        ...
        this._writer.write(frame).catch((err) => {
            this.onError?.(err);
    ...
    async _connectOnce() {
        ...
        try {
            ...
            this._transport = wt;
            wt.closed.then(() => this._finishClosed("webtransport_closed"), (err) => {
                this.onError?.(err);
