 ...
 */
export function encodeChatFrame(reqId, payload) {
    const body = enc.encode(payload);
    if (body.byteLength > MAX_FRAME_PAYLOAD) {
        throw new RangeError(`Chat frame payload too large: ${body.byteLength} bytes (max ${MAX_FRAME_PAYLOAD})`);
    }
    const buf = new ArrayBuffer(CHAT_FRAME_HEADER_LEN + body.byteLength);
    const view = new DataView(buf);
    view.setUint16(0, reqId, false /* big-endian */);
