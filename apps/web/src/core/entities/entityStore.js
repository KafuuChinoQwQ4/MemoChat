...
function richMessageKey(msg) {
    if (msg.serverMsgId !== undefined && msg.serverMsgId > 0)
        return `server:${msg.isGroup ? "g" : "p"}:${msg.serverMsgId}`;
    if (msg.clientMsgId)
...
function mergeMessages(existing, incoming) {
    ...
    for (const msg of existing)
        byKey.set(richMessageKey(msg), msg);
