...
export interface PersistedMessage extends RichMessage {
  _key: string // `${uid}_${peerId}_${clientMsgId}`
  _peerId: number
  _ownerUid: number
}

class ChatDatabase extends Dexie {
  messages!: EntityTable<PersistedMessage, "_key">
...
export async function hydrateMessages(ownerUid: number, peerId: number): Promise<RichMessage[]> {
...
export async function persistMessage(ownerUid: number, peerId: number, msg: RichMessage): Promise<void> {
...
export async function clearUserMessages(ownerUid: number): Promise<void> {
