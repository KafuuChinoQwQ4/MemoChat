...
import {
  ...
  MAX_AGENT_MESSAGES_PER_CONVERSATION,
  agentConversationStorageKey,
  loadAgentConversationState,
  sanitizeAgentConversationState,
...
function conversation(id: string, updatedAt: number, messageCount = 1): AgentConversation {
  return {
    id,
    title: `会话 ${id}`,
    updatedAt,
...
describe("agentConversationPersistence", () => {
  ...
  it("drops invalid messages and prunes old history", () => {
    const conversations = Array.from({ length: MAX_AGENT_CONVERSATIONS + 4 }, (_, index) =>
      conversation(`c-${index}`, index + 1, MAX_AGENT_MESSAGES_PER_CONVERSATION + 5),
    )
    const sanitized = sanitizeAgentConversationState({
      conversations: [
        ...conversations,
