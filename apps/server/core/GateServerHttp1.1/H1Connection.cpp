...
H1Connection::H1Connection(boost::asio::io_context& ioc)
    : _socket(ioc)
{
}

void H1Connection::SetFileResponse(const std::string& file_path, const std::string& content_type)
{
        ...
        [self](beast::error_code ec, std::size_t bytes_transferred)
        {
            try
            {
                ...
                self->_request_span.reset(
                    new memolog::SpanScope("GateServerHttp1.1.HttpRequest",
                                           "SERVER",
...
void H1Connection::PreParseGetParam()
{
...
void H1Connection::HandleReq()
{
    ...
    {
        PreParseGetParam();
        bool success = H1LogicSystem::GetInstance()->HandleGet(_get_url, shared_from_this());
        if (!success)
        {
        ...
        _response.set(http::field::server, "GateServerHttp1.1");
        WriteResponse();
        return;
    }
    ...
    {
        auto self = shared_from_this();
        H1WorkerPool::GetInstance()->post(
            [self]()
            {
                try
                {
                    auto* ioc = h1globals::g_main_ioc;
                    bool handled = H1LogicSystem::GetInstance()->HandlePost(self->_request.target(), self);
                    boost::asio::post(*ioc,
                                      [self, handled]()
                                      {
                                          if (!handled)
                                          ...
                                          self->_response.set(http::field::server, "GateServerHttp1.1");
                                          self->WriteResponse();
                                      });
                }
                catch (const std::exception& e)
                {
                    ...
                    if (auto* ioc = h1globals::g_main_ioc)
                    {
                        boost::asio::post(*ioc,
                                          [self]()
                                          {
...
void H1Connection::CheckDeadline()
{
    auto self = shared_from_this();
...
void H1Connection::FinishRequest(beast::error_code ec)
{
...
void H1Connection::WriteErrorResponse(http::status status, const std::string& message)
{
...
void H1Connection::WriteFileResponse()
{
    _send_file_response = false;
    ...
    response->set(http::field::server, "GateServerHttp1.1");
...
void H1Connection::WriteResponse()
{
    auto self = shared_from_this();
