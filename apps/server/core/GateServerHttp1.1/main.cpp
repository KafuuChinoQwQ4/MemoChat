...
int main()
{
    ...
    try
    {
        ...
        memolog::LogInfo("service.postgres_ready",
                         "GateServerHttp1.1 postgres ready",
                         {{"postgres_init_ms",
        ...
        memolog::LogInfo(
            "gate.thread_pool",
            "GateServerHttp1.1 thread pool configured",
        ...
        signals.async_wait(
            [&ioc](const boost::system::error_code& error, int)
            {
                if (!error)
                {
                    ioc.stop();
        ...
        memolog::LogInfo("service.start",
                         "GateServerHttp1.1 listening",
