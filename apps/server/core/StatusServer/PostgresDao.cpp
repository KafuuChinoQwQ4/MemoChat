...
std::string BuildConnectionString()
{
    ...
    return "host=" + host + " port=" + port + " user=" + user + " password=" + pwd + " dbname=" + database +
           " sslmode=" + (sslmode.empty() ? "disable" : sslmode) + " options=-csearch_path=" + schema + ",public";
}
} // namespace
...
int PostgresDao::RegUser(const std::string& name, const std::string& email, const std::string& pwd)
{
    ...
    try
    {
        ...
        txn.exec0("INSERT INTO \"user\"(uid, name, email, pwd, nick, \"desc\", sex, icon) VALUES (" + txn.quote(uid) +
                  ", " + txn.quote(name) + ", " + txn.quote(email) + ", " + txn.quote(pwd) + ", " + txn.quote(name) +
                  ", '', 0, ':/res/head_1.jpg')");
        txn.commit();
        pool_->returnConnection(std::move(con));
        return uid;
...
bool PostgresDao::CheckEmail(const std::string& name, const std::string& email)
{
    ...
    try
    {
        ...
        auto rows = txn.exec_params("SELECT 1 FROM \"user\" WHERE email = $1 LIMIT 1", email);
        const bool exists = !rows.empty();
        pool_->returnConnection(std::move(con));
        return exists;
    }
    catch (const std::exception& e)
...
bool PostgresDao::UpdatePwd(const std::string& email, const std::string& newpwd)
{
    ...
    try
    {
        ...
        pqxx::work txn(*con->_con);
        auto result = txn.exec_params("UPDATE \"user\" SET pwd = $1 WHERE email = $2", newpwd, email);
