                                               ...
                                               const std::string& event_type)
{
    ...
    {
        memolog::LogWarn("status.side_effect.audit_publish_failed",
                         "failed to publish audit login event",
                         {{"uid", std::to_string(uid)}, {"event_type", event_type}, {"error", error}});
    }
}

void StatusAsyncSideEffects::PublishPresenceRefresh(int uid,
                                                    const std::string& selected_server,
                                                    const std::string& reason)
{
    ...
    {
        memolog::LogWarn(
            "status.side_effect.presence_publish_failed",
            "failed to publish presence refresh task",
...
void StatusAsyncSideEffects::ConsumePresenceRefreshLoop()
{
    ...
    while (!_stop.load())
    {
        ...
        if (!connected)
        {
            ...
            memolog::LogWarn("status.presence_worker.connect_failed",
                             "failed to connect rabbitmq presence worker",
                             {{"error", error}});
        ...
        if (!consume_started)
        {
            amqp_basic_consume(static_cast<amqp_connection_state_t>(_rabbit_connection),
                               1,
                               amqp_cstring_bytes("status.presence.refresh.q"),
                               amqp_empty_bytes,
                               0,
                               0,
                               0,
...
bool StatusAsyncSideEffects::EnsureRabbitConnected(std::string* error)
{
    ...
    if (!StatusRpcReplyOk(amqp_login(connection,
                                     _rabbit_vhost.c_str(),
                                     0,
                                     131072,
                                     0,
...
bool StatusAsyncSideEffects::EnsureRabbitTopology(std::string* error)
{
    ...
    amqp_queue_bind(connection,
                    ...
                    amqp_cstring_bytes("status.presence.refresh.q"),
                    amqp_cstring_bytes(_rabbit_exchange_direct.c_str()),
                    amqp_cstring_bytes("status.presence.refresh"),
                    amqp_empty_table);
    if (!StatusRpcReplyOk(amqp_get_rpc_reply(connection), error))
    {
        return false;
...
void StatusAsyncSideEffects::HandlePresenceRefresh(const memochat::json::JsonValue& payload)
{
    ...
    if (!_known_servers.empty() && !selected_server.empty() &&
        _known_servers.find(selected_server) == _known_servers.end())
    {
        const auto uid_str = std::to_string(uid);
        RedisMgr::GetInstance()->Del(USERIPPREFIX + uid_str);
        memolog::LogWarn("status.presence_refresh.stale_route",
                         "removed stale presence route",
                         {{"uid", uid_str}, {"selected_server", selected_server}});
        return;
    }
    memolog::LogInfo("status.presence_refresh.processed",
                     "processed presence refresh task",
                     {{"uid", std::to_string(uid)},
                      {"selected_server", selected_server},
                      {"reason", memochat::json::glaze_safe_get<std::string>(payload, "reason", "")}});
}
