                                             ...
                                             std::string* error)
{
    ...
    const auto json_str = memochat::json::glaze_stringify(payload);
    memochat::json::JsonValue converted_payload;
    if (!json_str.empty())
    {
        memochat::json::JsonCharReaderBuilder builder;
        std::unique_ptr<memochat::json::JsonCharReader> reader(builder.newCharReader());
        std::string errors;
        reader->parse(json_str.data(), json_str.data() + json_str.size(), &converted_payload, &errors);
    }
    return _event_bus->Publish(topic, converted_payload, error);
}
...
void AsyncEventDispatcher::HandlePrivateAsyncEvent(const memochat::json::JsonValue& root)
{
    ...
    if (from_uid <= 0 || to_uid <= 0 || !text_array.is_array() || text_array.empty())
    {
        return;
    }
    ...
    notify_payload["text_array"] = memochat::json::array_t{};
    ...
    for (const auto& txt_obj : text_array)
    {
        ...
        append(notify_payload["text_array"], one);
    }
    ...
    {
        const auto notify_rsp = ChatGrpcClient::GetInstance()->NotifyTextChatMsg(route.redis_server, text_msg_req);
    ...
    memochat::json::JsonValue dialog_sync(memochat::json::object_t{});
...
void AsyncEventDispatcher::HandleRelationStateEvent(const memochat::json::JsonValue& root)
{
    ...
    if (isMember(root, "uids") && root["uids"].is_array())
    {
...
void AsyncEventDispatcher::NotifyMessageStatus(const memochat::json::JsonValue& payload)
{
    memochat::json::JsonValue converted = ConvertGlazeToJson(payload);
    NotifyMessageStatus(converted);
}

void AsyncEventDispatcher::HandlePrivateAsyncEvent(const memochat::json::JsonValue& root)
{
    memochat::json::JsonValue converted = ConvertGlazeToJson(root);
    HandlePrivateAsyncEvent(converted);
}

void AsyncEventDispatcher::HandleGroupAsyncEvent(const memochat::json::JsonValue& root)
{
    memochat::json::JsonValue converted = ConvertGlazeToJson(root);
    HandleGroupAsyncEvent(converted);
}

void AsyncEventDispatcher::HandleDialogSyncEvent(const memochat::json::JsonValue& root)
{
    memochat::json::JsonValue converted = ConvertGlazeToJson(root);
    HandleDialogSyncEvent(converted);
}

void AsyncEventDispatcher::HandleRelationStateEvent(const memochat::json::JsonValue& root)
{
    memochat::json::JsonValue converted = ConvertGlazeToJson(root);
    HandleRelationStateEvent(converted);
}
