...
std::shared_ptr<CSession> UserMgr::FindSession(int uid)
{
    if (uid <= 0)
    {
        memolog::LogWarn("usermgr.get_session.invalid", "invalid uid", {{"uid", std::to_string(uid)}});
        return nullptr;
    }
    ...
    if (iter == _uid_to_session.end())
    {
        memolog::LogInfo("usermgr.get_session",
                         "session not found in local map",
    ...
    if (!iter->second)
    {
        memolog::LogWarn("usermgr.get_session",
                         "session is null",
                         {{"uid", std::to_string(uid)}, {"map_size", std::to_string(_uid_to_session.size())}});
...
void UserMgr::BindSession(int uid, std::shared_ptr<CSession> session)
{
    if (uid <= 0 || !session)
    {
        memolog::LogWarn("usermgr.set_session.invalid", "invalid params", {{"uid", std::to_string(uid)}});
        return;
    }
    std::lock_guard<std::mutex> lock(_session_mtx);
...
void UserMgr::UnbindSession(int uid, const std::string& session_id)
{
    ...
    {
        std::lock_guard<std::mutex> lock(_session_mtx);
        memolog::LogInfo("usermgr.rmv_session",
                         "removing session",
                         {{"uid", std::to_string(uid)}, {"session_id", session_id}});
        auto iter = _uid_to_session.find(uid);
        if (iter == _uid_to_session.end())
        {
            memolog::LogInfo("usermgr.rmv_session", "session not found", {{"uid", std::to_string(uid)}});
            return;
        }
        ...
        memolog::LogInfo("usermgr.rmv_session", "session removed", {{"uid", std::to_string(uid)}});
    }
}
