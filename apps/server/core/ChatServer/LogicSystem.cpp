    ...
    , _p_server(nullptr)
{
    ...
    _message_delivery_service = std::make_unique<MessageDeliveryService>(
        [this](const std::string& task_type,
    ...
    _async_event_dispatcher = std::make_unique<AsyncEventDispatcher>(
        ...
        [this](const std::vector<int>& recipients,
               short msgid,
               const memochat::json::JsonValue& payload,
               int exclude_uid)
        {
...
void LogicSystem::GetRelationBootstrapHandler(std::shared_ptr<CSession> session,
                                              const short& msg_id,
...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    std::cout << "user login uid is  " << uid << " applyname  is " << applyname << " bakname is " << bakname
              << " touid is " << touid << endl;
...
void LogicSystem::DealChatTextMsg(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    if (notify_rsp.error() != ErrorCodes::Success)
    {
        ...
        LogPrivateRoute("chat.private.route",
                        uid,
                        touid,
                        first_msg_id,
                        route,
...
void LogicSystem::GetUserByUid(std::string uid_str, memochat::json::JsonValue& rtvalue)
{
    ...
    if (b_base)
    {
        ...
        std::cout << "user  uid is  " << uid << " name  is " << name << " pwd is " << pwd << " email is " << email
                  << " icon is " << icon << endl;
...
bool LogicSystem::GetBaseInfo(std::string base_key, int uid, std::shared_ptr<UserInfo>& userinfo)
{
    ...
    if (b_base)
    {
        ...
        std::cout << "user login uid is  " << userinfo->uid << " name  is " << userinfo->name << " pwd is "
                  << userinfo->pwd << " email is " << userinfo->email << endl;
    }
    else
    {
        std::shared_ptr<UserInfo> user_info = nullptr;
...
void LogicSystem::ForwardPrivateMsgHandler(std::shared_ptr<CSession> session,
                                           const short& msg_id,
                                           const string& msg_data)
{
    ...
    if (notify_rsp.error() != ErrorCodes::Success)
    {
        ...
        LogPrivateRoute("chat.private.forward.route",
                        from_uid,
                        peer_uid,
                        info.msg_id,
                        route,
...
void LogicSystem::PushGroupPayload(const std::vector<int>& recipients,
                                   short msgid,
                                   const memochat::json::JsonValue& payload,
                                   int exclude_uid)
{
...
void LogicSystem::InviteGroupMemberHandler(std::shared_ptr<CSession> session,
                                           const short& msg_id,
                                           const string& msg_data)
{
    if (_group_message_service)
...
void LogicSystem::ReviewGroupApplyHandler(std::shared_ptr<CSession> session,
                                          const short& msg_id,
                                          const string& msg_data)
{
    if (_group_message_service)
...
void LogicSystem::DealGroupChatMsg(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    std::vector<std::shared_ptr<GroupMemberInfo>> members;
    PostgresMgr::GetInstance()->GetGroupMemberList(group_id, members);
    const auto now_sec = static_cast<int64_t>(
    ...
    info.mentions_json = msg.get("mentions", memochat::json::array_t{})
                             .and_then(
                                 [](auto&& v)
                                 {
                                     return glz::write_json(v);
    ...
    {
        info.forward_meta_json = msg["forward_meta"]
                                     .and_then(
                                         [](auto&& v)
                                         {
                                             return glz::write_json(v);
...
void LogicSystem::EditPrivateMsgHandler(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    if (MongoMgr::GetInstance()->Enabled() &&
        !MongoMgr::GetInstance()->UpdatePrivateMessageContent(uid, peer_uid, target_msg_id, content, now_ms))
    {
...
void LogicSystem::RevokePrivateMsgHandler(std::shared_ptr<CSession> session,
                                          const short& msg_id,
                                          const string& msg_data)
{
    if (_private_message_service)
    ...
    if (MongoMgr::GetInstance()->Enabled() &&
        !MongoMgr::GetInstance()->RevokePrivateMessage(uid, peer_uid, target_msg_id, now_ms))
    {
...
void LogicSystem::EditGroupMsgHandler(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    if (MongoMgr::GetInstance()->Enabled() &&
        !MongoMgr::GetInstance()->UpdateGroupMessageContent(group_id, uid, target_msg_id, content, now_ms))
    {
...
void LogicSystem::RevokeGroupMsgHandler(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    if (MongoMgr::GetInstance()->Enabled() &&
        !MongoMgr::GetInstance()->RevokeGroupMessage(group_id, uid, target_msg_id, now_ms))
    {
...
void LogicSystem::UpdateGroupAnnouncementHandler(std::shared_ptr<CSession> session,
                                                 const short& msg_id,
                                                 const string& msg_data)
{
    if (_group_message_service)
