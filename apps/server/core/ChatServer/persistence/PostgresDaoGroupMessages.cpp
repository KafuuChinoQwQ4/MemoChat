                                   ...
                                   int64_t assigned_group_seq)
{
    ...
    try
    {
        ...
        {
            std::unique_ptr<sql::PreparedStatement> lock_group_stmt(con->_con->prepareStatement(
                "SELECT group_id FROM chat_group WHERE group_id = ? AND status = 1 LIMIT 1 FOR UPDATE"));
                                            ...
                                            int64_t edited_at_ms)
{
    ...
    try
    {
        ...
        std::unique_ptr<sql::PreparedStatement> up(con->_con->prepareStatement(
            "UPDATE chat_group_msg SET content = ?, msg_type = 'text', mentions_json = '[]', "
