...
public:
    ...
    bool MarkChatOutboxEventRetry(int64_t id,
                                  int retry_count,
                                  int64_t next_retry_at_ms,
                                  const std::string& last_error,
                                  bool terminal_error);
    bool ExpediteChatOutboxEventRetry(int64_t id);
    bool GetPrivateHistory(const int& uid,
                           const int& peer_uid,
    ...
    bool UpdatePrivateMessageContent(const int& uid,
                                     const int& peer_uid,
    ...
    bool InviteGroupMember(const int64_t& group_id,
    ...
    bool ReviewGroupApply(const int64_t& apply_id,
    ...
    bool SaveGroupMessage(const GroupMessageInfo& msg,
                          ...
                          int64_t assigned_group_seq = 0);
    bool UpdateGroupMessageContent(const int64_t& group_id,
                                   const int& operator_uid,
                                   const std::string& msg_id,
                                   const std::string& content,
                                   int64_t edited_at_ms = 0);
    bool RevokeGroupMessage(const int64_t& group_id,
                            const int& operator_uid,
                            const std::string& msg_id,
                            int64_t deleted_at_ms = 0);
    bool GetGroupHistory(const int64_t& group_id,
                         const int64_t& before_ts,
    ...
    bool SetGroupAdmin(const int64_t& group_id,
                       const int& operator_uid,
                       const int& target_uid,
                       const bool& is_admin,
                       const int64_t& permission_bits = 0);
    bool
    MuteGroupMember(const int64_t& group_id, const int& operator_uid, const int& target_uid, const int64_t& mute_until);
    ...
    bool GetPendingGroupApplyForReviewer(const int& reviewer_uid,
    ...
    bool GetGroupMessageByMsgId(const int64_t& group_id,
                                const std::string& msg_id,
                                std::shared_ptr<GroupMessageInfo>& message);
    bool UpsertDialogDraft(const int& owner_uid,
                           const std::string& dialog_type,
                           const int& peer_uid,
                           const int64_t& group_id,
                           const std::string& draft_text);
    bool UpsertDialogPinned(const int& owner_uid,
                            const std::string& dialog_type,
                            const int& peer_uid,
                            const int64_t& group_id,
                            const int& pinned_rank);
    bool UpsertDialogMuteState(const int& owner_uid,
                               const std::string& dialog_type,
                               const int& peer_uid,
                               const int64_t& group_id,
                               const int& mute_state);
    bool GetUndeliveredPrivateMessages(const int& to_uid,
