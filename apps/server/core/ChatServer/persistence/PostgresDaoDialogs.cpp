...
bool PostgresDao::GetDialogMetaByOwner(const int& owner_uid, std::vector<std::shared_ptr<DialogMetaInfo>>& metas)
{
    ...
    if (use_postgres_)
    {
        try
        {
            ...
            const auto rows =
                txn.exec_params("SELECT dialog_type, peer_uid, group_id, pinned_rank, draft_text, mute_state "
    ...
    try
    {
        std::unique_ptr<sql::PreparedStatement> pstmt(
            con->_con->prepareStatement("SELECT dialog_type, peer_uid, group_id, pinned_rank, draft_text, mute_state "
...
bool PostgresDao::GetGroupDialogRuntime(const int& owner_uid, const int64_t& group_id, DialogRuntimeInfo& runtime)
{
    ...
    {
        std::unique_ptr<sql::PreparedStatement> pstmt(
            con->_con->prepareStatement("SELECT last_msg_preview, last_msg_ts, unread_count FROM chat_dialog "
...
bool PostgresDao::RefreshDialogsForOwner(const int& owner_uid)
{
    ...
    if (use_postgres_)
    {
        try
        {
            ...
            for (int peer_uid : peers)
            {
                ...
                {
                    const auto content =
                        last_rows[0]["content"].is_null() ? "" : std::string(last_rows[0]["content"].c_str());
            ...
            for (int64_t group_id : groups)
            {
                ...
                const auto unread_rows =
                    txn.exec_params("SELECT COUNT(1) AS unread_count FROM chat_group_msg "
    ...
    try
    {
        ...
        for (int peer_uid : peers)
        {
            std::unique_ptr<sql::PreparedStatement> ensure_row(con->_con->prepareStatement(
                "INSERT INTO chat_dialog(owner_uid, dialog_type, peer_uid, group_id, pinned_rank, draft_text, "
                "mute_state, last_msg_preview, last_msg_ts, unread_count) "
...

            {
                std::unique_ptr<sql::PreparedStatement> last_stmt(
                    con->_con->prepareStatement("SELECT content, created_at FROM chat_private_msg "
...

            {
                std::unique_ptr<sql::PreparedStatement> unread_stmt(
                    con->_con->prepareStatement("SELECT COUNT(1) AS unread_count FROM chat_private_msg "
                                                "WHERE conv_uid_min = ? AND conv_uid_max = ? AND from_uid = ? "
                                                "AND created_at > COALESCE((SELECT read_ts FROM "
                                                "chat_private_read_state WHERE uid = ? AND peer_uid = ?), 0)"));
            ...
            std::unique_ptr<sql::PreparedStatement> update_stmt(con->_con->prepareStatement(
                "UPDATE chat_dialog SET last_msg_preview = ?, last_msg_ts = ?, unread_count = ?, updated_at = "
                "CURRENT_TIMESTAMP "
        ...
        std::vector<int64_t> groups;
        {
            std::unique_ptr<sql::PreparedStatement> list_groups(
                con->_con->prepareStatement("SELECT m.group_id FROM chat_group_member m "
                                            "JOIN chat_group g ON g.group_id = m.group_id "
        ...
        for (int64_t group_id : groups)
        {
            ...
            {
                std::unique_ptr<sql::PreparedStatement> last_stmt(con->_con->prepareStatement(
            ...
            {
                std::unique_ptr<sql::PreparedStatement> unread_stmt(
                    con->_con->prepareStatement("SELECT COUNT(1) AS unread_count FROM chat_group_msg "
                                                "WHERE group_id = ? AND created_at > COALESCE((SELECT read_ts FROM "
                                                "chat_group_read_state WHERE uid = ? AND group_id = ?), 0) "
                                                "AND from_uid <> ?"));
            ...
            std::unique_ptr<sql::PreparedStatement> update_stmt(
                con->_con->prepareStatement("UPDATE chat_dialog SET last_msg_preview = ?, last_msg_ts = ?, "
                                            "unread_count = ?, updated_at = CURRENT_TIMESTAMP "
        ...
        {
            std::unique_ptr<sql::PreparedStatement> clean_private(con->_con->prepareStatement(
                "DELETE FROM chat_dialog "
        ...
        {
            std::unique_ptr<sql::PreparedStatement> clean_group(con->_con->prepareStatement(
                "DELETE FROM chat_dialog "
...
bool PostgresDao::UpsertGroupReadState(const int& uid, const int64_t& group_id, const int64_t& read_ts)
{
    ...
    if (use_postgres_)
    {
        try
        {
            ...
            pqxx::work txn(conn);
            const auto result =
                txn.exec_params("INSERT INTO chat_group_read_state(uid, group_id, read_ts) VALUES($1, $2, $3) "
                                    ...
                                    const std::string& draft_text)
{
    ...
    {
        std::unique_ptr<sql::PreparedStatement> pstmt(con->_con->prepareStatement(
            "INSERT INTO chat_dialog(owner_uid, dialog_type, peer_uid, group_id, pinned_rank, draft_text, mute_state, "
            "last_msg_preview, last_msg_ts, unread_count) "
            "VALUES(?, ?, ?, ?, 0, ?, 0, '', 0, 0) "
...
bool PostgresDao::EnsureChatMessageIdempotencySchema()
{
    ...
    try
    {
        ...
        txn.exec0(
            "CREATE UNIQUE INDEX IF NOT EXISTS uk_chat_group_msg_group_msg_id ON chat_group_msg(group_id, msg_id)");
        txn.commit();
        return true;
    }
    catch (const std::exception& e)
...
bool PostgresDao::EnsureChatEventOutboxSchema()
{
    ...
    try
    {
        ...
        txn.exec0("CREATE INDEX IF NOT EXISTS idx_chat_event_outbox_status_retry ON chat_event_outbox(status, "
                  "next_retry_at, id)");
        txn.exec0(
            "CREATE INDEX IF NOT EXISTS idx_chat_event_outbox_topic_status ON chat_event_outbox(topic, status, id)");
        txn.commit();
        return true;
    }
    catch (const std::exception& e)
...
bool PostgresDao::EnsureDialogMetaSchema()
{
    ...
    try
    {
        ...
        stmt->execute(
            ...
            "pinned_rank = CASE WHEN chat_dialog.pinned_rank = 0 THEN VALUES(pinned_rank) ELSE chat_dialog.pinned_rank "
            "END, "
            "draft_text = CASE WHEN chat_dialog.draft_text = '' THEN VALUES(draft_text) ELSE chat_dialog.draft_text "
            "END, "
            "mute_state = CASE WHEN chat_dialog.mute_state = 0 THEN VALUES(mute_state) ELSE chat_dialog.mute_state "
            "END");
        return true;
    }
    catch (sql::SQLException& e)
...
bool PostgresDao::EnsureGroupMessageOrderSchema()
{
    ...
    try
    {
        ...
        stmt->execute(
            ...
            "FROM (SELECT msg_id, group_id FROM chat_group_msg ORDER BY group_id ASC, created_at ASC, msg_id ASC) "
            "ordered"
            ") ranked");
        stmt->execute("UPDATE chat_group_msg m "
        ...
        ExecuteIgnoreSql(stmt.get(),
                         "CREATE UNIQUE INDEX uk_chat_group_msg_group_seq ON chat_group_msg(group_id, group_seq)");
        return true;
    }
    catch (sql::SQLException& e)
    {
...
bool PostgresDao::EnsureGroupPermissionSchemaAndBackfill()
{
    ...
    try
    {
        ...
        {
            stmt->execute(
                "ALTER TABLE chat_group_admin_permission ADD COLUMN permission_bits BIGINT NOT NULL DEFAULT 0");
        }
        catch (sql::SQLException&)
        {
        }
        ...
        std::unique_ptr<sql::PreparedStatement> upsert_owner(con->_con->prepareStatement(
        ...
        std::unique_ptr<sql::PreparedStatement> upsert_admin(
            con->_con->prepareStatement("INSERT INTO chat_group_admin_permission(group_id, uid, permission_bits) "
                                        ...
                                        "ON DUPLICATE KEY UPDATE permission_bits = IF(permission_bits <= 0, "
                                        "VALUES(permission_bits), permission_bits), updated_at = CURRENT_TIMESTAMP"));
...
bool PostgresDao::EnsureGroupCodeSchemaAndBackfill()
{
    ...
    try
    {
        ...
        std::unique_ptr<sql::PreparedStatement> ensure_owner_stmt(con->_con->prepareStatement(
        ...
        con->_con->commit();
        std::cout << "[ChatServer] group_code backfill done. total=" << all_rows.size() << " backfilled=" << backfilled
                  << " duplicate_fixed=" << duplicate_fixed << " retries=" << retries
                  << " owner_role_fixed=" << owner_role_fixed << " demoted_extra_owner=" << demoted_extra_owner
                  << std::endl;
        return true;
    }
    catch (sql::SQLException& e)
