                                              ...
                                              message::GetVarifyRsp* reply)
{
    ...
    if (!IsValidEmail(email))
    {
        ...
        memolog::LogWarn("varify.get_code.invalid_email",
    ...
    if (email_result == RateLimiter::Result::RateLimited)
    {
        ...
        memolog::LogWarn("varify.get_code.rate_limited_email",
    ...
    if (!ip.empty())
    {
        ...
        {
            g_metrics.rate_limited.fetch_add(1, std::memory_order_relaxed);
            memolog::LogWarn("varify.get_code.rate_limited_ip",
                             "rate limited by IP",
    ...
    if (!code_exists)
    {
        ...
        if (!stored)
        {
            ...
            memolog::LogError("varify.get_code.redis_failed",
                              "failed to store verify code in Redis",
                              {{"email", email}, {"trace_id", trace_id}});
            reply->set_error(static_cast<int>(VarifyError::RedisErr));
            return grpc::Status::OK;
        }
        memolog::LogInfo("varify.get_code.new_code",
                         "generated and stored new verify code",
                         {{"email", email}, {"trace_id", trace_id}});
    }
    else
    {
        memolog::LogInfo("varify.get_code.existing_code",
                         "reusing existing verify code",
                         {{"email", email}, {"trace_id", trace_id}});
    }
    ...
    {
        memolog::LogInfo("varify.get_code.synthetic_skipped",
                         "skipping email for synthetic loadtest account",
                         {{"email", email}, {"trace_id", trace_id}});
    }
