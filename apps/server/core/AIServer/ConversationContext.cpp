...
void ConversationContext::AddUserMessage(const std::string& content)
{
    ...
    m.role = "user";
    m.content = content;
    m.created_at = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
...
std::string ConversationContext::ToJson() const
{
    ...
    {
        auto jm = memochat::json::make_document(memochat::json::kvp<std::string>("msg_id", m.msg_id),
    ...
    auto root =
        memochat::json::make_document(memochat::json::kvp<std::string>("session_id", session_id),
