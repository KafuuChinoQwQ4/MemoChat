...
public:
    Impl()
    {
        ...
        _timeout_sec =
            std::stoi(cfg["AIOrchestrator"]["TimeoutSec"].empty() ? "300" : cfg["AIOrchestrator"]["TimeoutSec"]);
    }
...
{
    return _impl->DeleteJson("/kb/" + AIServiceClient::Impl::UrlEncode(kb_id) + "?uid=" + std::to_string(uid),
                             out_result);
}
...
{
    return _impl->DeleteJson("/agent/memory/" + AIServiceClient::Impl::UrlEncode(memory_id) +
...
{
    return _impl->GetJson("/agent/tasks?uid=" + std::to_string(uid) +
                              "&limit=" + std::to_string(limit > 0 ? limit : 50),
                          out_result);
}

grpc::Status AIServiceClient::AgentTaskGet(const std::string& task_id, json::JsonValue* out_result)
{
    return _impl->GetJson("/agent/tasks/" + AIServiceClient::Impl::UrlEncode(task_id), out_result);
}

grpc::Status AIServiceClient::AgentTaskCancel(const std::string& task_id, json::JsonValue* out_result)
{
    return _impl->PostJson("/agent/tasks/" + AIServiceClient::Impl::UrlEncode(task_id) + "/cancel",
                           json::JsonValue{},
                           out_result);
}

grpc::Status AIServiceClient::AgentTaskResume(const std::string& task_id, json::JsonValue* out_result)
{
    return _impl->PostJson("/agent/tasks/" + AIServiceClient::Impl::UrlEncode(task_id) + "/resume",
                           json::JsonValue{},
                           out_result);
}
