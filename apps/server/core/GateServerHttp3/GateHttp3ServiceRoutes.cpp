                  ...
                  [](std::shared_ptr<GateHttp3Connection> connection)
                  {
                      return GateHttp3JsonSupport::HandleJsonPost(
                             ...
                             const std::string& trace_id)
                          {
                              ...
                              memolog::LogInfo("gate.http3.get_varifycode",
                                               "HTTP/3 verify code requested",
                      ...
                      return GateHttp3JsonSupport::HandleJsonPost(
                             ...
                             const std::string& trace_id)
                          {
                              ...
                              GateAsyncSideEffects::Instance().PublishUserProfileChanged(uid,
                                                                                         ...
                                                                                         icon,
                                                                                         cached_user.sex);
                              memolog::LogInfo("gate.http3.user_register",
                                               "HTTP/3 user registered",
            ...
            return GateHttp3JsonSupport::HandleJsonPost(
                   ...
                   const std::string& trace_id)
                {
                    ...
                    if (!gateauthsupport::IsClientVersionAllowed(client_ver, gateauthsupport::MinClientVersion()))
                    {
                        ...
                        memolog::LogWarn(
                            "gate.http3.user_login.version_low",
                    ...
                    if (!pwd_valid)
                    {
                        memolog::LogWarn("gate.http3.user_login.pwd_invalid",
                                         "password invalid",
                    ...
                    {
                        memolog::LogWarn("gate.http3.user_login.no_server",
                    ...
                    if (http_token.empty() &&
                        (!RedisMgr::GetInstance()->Get(token_key, http_token) || http_token.empty()))
                    {
                    ...
                    claims.expire_at_ms =
                        claims.issued_at_ms + static_cast<int64_t>(gateauthsupport::GetChatTicketTtlSec()) * 1000;
                    const std::string login_ticket =
                        memochat::auth::EncodeTicket(claims, gateauthsupport::GetChatAuthSecret());
                    ...
                    root["preferred_transport"] =
                        (!route_nodes.front().quic_host.empty() && !route_nodes.front().quic_port.empty()) ? "quic"
                                                                                                           : "tcp";
                    ...
                    memolog::LogInfo("gate.http3.user_login",
                                     "HTTP/3 user login succeeded",
                                     {{"uid", std::to_string(userInfo.uid)},
                                      {"email", email},
                                      {"route", "/user_login"},
                                      {"trace_id", trace_id}});
