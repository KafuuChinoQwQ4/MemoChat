...
bool GenerateRSAKey(EVP_PKEY** out_pkey)
{
    *out_pkey = nullptr;
    ...
    if (EVP_PKEY_keygen_init(ctx) != 1 || EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, 2048) != 1 ||
...
bool GenerateSelfSignedCertPem(const std::string& crt_path, const std::string& key_path)
{
    ...
    {
        memolog::LogError("certutil.key_write.fail",
    ...
    {
        memolog::LogError("certutil.crt_write.fail",
...
bool WritePKCS12(const std::string& pfx_path, const std::string& password, EVP_PKEY* pkey, X509* x509)
{
    FILE* pfx_fp = nullptr;
    if (!OpenFile(&pfx_fp, pfx_path.c_str(), "wb"))
    {
        memolog::LogError("certutil.pfx_file.fail", "failed to open PFX file", {{"path", pfx_path}});
        return false;
    }

    PKCS12* p12 = PKCS12_create(password.c_str(), nullptr, pkey, x509, nullptr, 0, 0, -1, -1, 0);
    if (!p12)
    {
    ...
    {
        memolog::LogError("certutil.pfx_write.fail",
                          "failed to write PFX",
                          {{"openssl_error", std::to_string(ERR_get_error())}});
        PKCS12_free(p12);
        fclose(pfx_fp);
                               ...
                               const std::string& password)
{
    ...
    if (ok)
        ok = WriteCertificate(crt_path, x509);
    if (ok)
        ok = WritePKCS12(pfx_path, password, pkey, x509);

    if (ok)
    {
        memolog::LogInfo("certutil.pfx.ok",
                         "self-signed PEM+PFX cert/key generated",
