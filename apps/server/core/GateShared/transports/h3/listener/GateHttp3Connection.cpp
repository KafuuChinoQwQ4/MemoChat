...
void GateHttp3Connection::OnStreamOpen(int64_t stream_id)
{
    ...
    memolog::LogInfo("http3.stream.open",
                     "HTTP/3 stream opened",
                     {{"stream_id", std::to_string(stream_id)}, {"trace_id", trace_id_}});
}

void GateHttp3Connection::OnStreamClose(int64_t stream_id, uint64_t)
{
    stream_open_ = false;
    memolog::LogInfo("http3.stream.close", "HTTP/3 stream closed", {{"stream_id", std::to_string(stream_id)}});
...
void GateHttp3Connection::OnStreamRead(int64_t, const uint8_t* data, size_t len)
{
    ...
    if (content_length_ >= 0 && static_cast<int64_t>(request_body_received_) >= content_length_)
    {
        body_complete_ = true;
    }
}
                                       ...
                                       const std::unordered_map<std::string, std::string>& headers)
{
    ...
    memolog::LogInfo(
        ...
        "HTTP/3 response prepared",
        {{"status", std::to_string(status)}, {"body_len", std::to_string(body.size())}, {"trace_id", trace_id_}});
    return true;
}
