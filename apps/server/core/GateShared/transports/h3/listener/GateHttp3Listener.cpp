...
struct GateHttp3Listener::Impl : public std::enable_shared_from_this<Impl>
{
    ...
    Impl(boost::asio::io_context& ioc, LogicSystem& logic, int port, std::atomic<bool>& running)
        : ioc_(ioc)
        , logic_(logic)
        , port_(port)
        , running_(running)
...
static QUIC_STATUS ListenerCallback(HQUIC, void* Context, QUIC_LISTENER_EVENT* Event)
{
    ...
    switch (Event->Type)
    {
        case QUIC_LISTENER_EVENT_NEW_CONNECTION:
        {
            ...
            g_QuicApi->SetCallbackHandler(Event->NEW_CONNECTION.Connection,
            ...
            {
                memcpy(alpn, info->NegotiatedAlpn, std::min((size_t) info->NegotiatedAlpnLength, sizeof(alpn) - 1));
            }
            std::map<std::string, std::string> fields;
            fields["alpn"] = alpn;
            fields["trace_id"] = conn_ctx->TraceId;
            memolog::LogInfo("http3.conn.incoming", "HTTP/3 incoming connection", fields);

            QUIC_STATUS status =
                g_QuicApi->ConnectionSetConfiguration(Event->NEW_CONNECTION.Connection, impl->Configuration);
            if (QUIC_FAILED(status))
            {
                std::map<std::string, std::string> efields;
                efields["status"] = std::to_string(status);
                memolog::LogError("http3.conn.config.fail", "Failed to set configuration", efields);
                delete conn_ctx;
            }
            return QUIC_STATUS_SUCCESS;
        }
        case QUIC_LISTENER_EVENT_STOP_COMPLETE:
            memolog::LogInfo("http3.listener.stopped", "HTTP/3 listener stopped", {});
            return QUIC_STATUS_SUCCESS;
        default:
...
static QUIC_STATUS ConnectionCallback(HQUIC Connection, void* Context, QUIC_CONNECTION_EVENT* Event)
{
    ...
    switch (Event->Type)
    {
        case QUIC_CONNECTION_EVENT_CONNECTED:
        {
            ...
            std::map<std::string, std::string> fields;
            fields["trace_id"] = tid;
            memolog::LogInfo("http3.conn.connected", "HTTP/3 connection established", fields);
            return QUIC_STATUS_SUCCESS;
        }
        case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT:
        {
            ...
            fields["trace_id"] = tid;
            memolog::LogWarn("http3.conn.transport_shutdown", "Connection shutdown by transport", fields);
            return QUIC_STATUS_SUCCESS;
        }
        case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_PEER:
        {
            ...
            std::map<std::string, std::string> fields;
            fields["trace_id"] = tid;
            memolog::LogWarn("http3.conn.peer_shutdown", "Connection shutdown by peer", fields);
            return QUIC_STATUS_SUCCESS;
        }
        ...
        case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED:
        {
            ...
            g_QuicApi->SetCallbackHandler(Event->PEER_STREAM_STARTED.Stream,
                                          reinterpret_cast<void*>(StreamCallback),
                                          new_ctx);
            std::map<std::string, std::string> fields;
            fields["trace_id"] = new_ctx->TraceId;
            memolog::LogInfo("http3.stream.started", "HTTP/3 stream started", fields);
            return QUIC_STATUS_SUCCESS;
        }
        default:
            return QUIC_STATUS_SUCCESS;
    ...
    switch (Event->Type)
    {
            ...
            switch (stream_ctx->State)
            {
                case StreamContext::STATE_READING_HEADERS:
                {
                    ...
                    for (uint32_t i = 0; i < buffer_count; ++i)
                    {
                        ...
                        stream_ctx->HeadersRaw.append(reinterpret_cast<const char*>(data), len);
                    ...
                    if (header_end == std::string::npos)
                    {
                        ...
                        memolog::LogDebug("http3.stream.header_partial", "HTTP/3 partial header received", fields);
                        break;
                    }
                ...
                case StreamContext::STATE_READING_BODY:
                {
                    ...
                    for (uint32_t i = 0; i < buffer_count; ++i)
                    {
                        stream_ctx->Body.append(reinterpret_cast<const char*>(buffers[i].Buffer), buffers[i].Length);
                    }
        ...
        do_dispatch:
            ...
            if (stream_ctx->Method == "POST" || stream_ctx->Method == "post")
            {
                ...
                if (!handled)
                {
                    ...
                    memolog::LogWarn("http3.route.not_found", "HTTP/3 POST route not found", f);
                }
            }
            else
            {
                ...
                if (!handled)
                {
                    ...
                    f["path"] = path;
                    f["trace_id"] = stream_ctx->TraceId;
                    memolog::LogWarn("http3.route.not_found", "HTTP/3 GET route not found", f);
                }
            }
            ...
            QUIC_STATUS send_st = g_QuicApi->StreamSend(Stream, &qbuf, 1, QUIC_SEND_FLAG_FIN, nullptr);
            if (QUIC_SUCCEEDED(send_st))
            {
                ...
                f["trace_id"] = stream_ctx->TraceId;
                memolog::LogInfo("http3.response.sent", "HTTP/3 response sent", f);
            }
            else
            {
                std::map<std::string, std::string> f;
                ...
                f["trace_id"] = stream_ctx->TraceId;
                memolog::LogWarn("http3.response.send_fail", "HTTP/3 response send failed", f);
            }
        ...
        case QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN:
        {
            ...
            if (stream_ctx && !stream_ctx->BodyComplete && !stream_ctx->ResponseSent)
            {
                ...
                memolog::LogInfo("http3.stream.peer_shutdown", "HTTP/3 peer send shutdown, dispatching", f);
        ...
        case QUIC_STREAM_EVENT_PEER_SEND_ABORTED:
        {
            std::map<std::string, std::string> f;
            f["trace_id"] = stream_ctx->TraceId;
            memolog::LogWarn("http3.stream.aborted", "HTTP/3 stream aborted by peer", f);
            return QUIC_STATUS_SUCCESS;
        }
        case QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE:
        {
            std::map<std::string, std::string> f;
            f["trace_id"] = stream_ctx->TraceId;
            memolog::LogInfo("http3.stream.closed", "HTTP/3 stream closed", f);
            delete stream_ctx;
            return QUIC_STATUS_SUCCESS;
        }
        default:
...
static bool ValidatePfxReadable(const std::string& pfx_path)
{
    ...
    std::vector<uint8_t> data((std::istreambuf_iterator<char>(pfx_file)), std::istreambuf_iterator<char>());
    pfx_file.close();
...
static bool LoadSelfSignedCertFromPfx(HQUIC Configuration, const std::string& pfx_path, std::string& error_out)
{
    ...
    std::vector<uint8_t> pfx_data((std::istreambuf_iterator<char>(pfx_file)), std::istreambuf_iterator<char>());
    pfx_file.close();
    ...
    memolog::LogInfo("http3.cert.pfx.ok", "Self-signed certificate loaded from PFX for HTTP/3", {{"pfx", pfx_path}});
    return true;
}

GateHttp3Listener::GateHttp3Listener(boost::asio::io_context& ioc, LogicSystem& logic, int port)
    : ioc_(ioc)
    , logic_(logic)
    , port_(port)
#if MEMOCHAT_ENABLE_HTTP3
...
GateHttp3Listener::~GateHttp3Listener()
{
    Stop();
}
...
bool GateHttp3Listener::Start(std::string& error)
{
    ...
    if (!g_QuicApi)
    {
        ...
        if (QUIC_FAILED(st))
        {
            ...
            memolog::LogError("http3.msquic.fail", error, {});
            return false;
        }
    ...
    if (!g_Registration)
    {
        QUIC_REGISTRATION_CONFIG cfg{"GateServer-H3", QUIC_EXECUTION_PROFILE_LOW_LATENCY};
        ...
        memolog::LogInfo("http3.reg.ok", "MsQuic registration opened", {});
    }
    ...
    QUIC_STATUS st =
        g_QuicApi
            ->ConfigurationOpen(g_Registration, alpns, 3, &settings, sizeof(settings), nullptr, &pImpl_->Configuration);
    if (QUIC_FAILED(st))
    {
    ...
    st = g_QuicApi->ConfigurationLoadCredential(pImpl_->Configuration, &cred);
    if (QUIC_FAILED(st))
    {
        ...
        memolog::LogWarn("http3.cred.pfx.fallback", "Successfully loaded certificate via PFX fallback", {});
    }

    pImpl_->listener_ = this;

    st = g_QuicApi->ListenerOpen(g_Registration, ListenerCallback, pImpl_.get(), &pImpl_->Listener);
    if (QUIC_FAILED(st))
    {
    ...
    if (QUIC_FAILED(st))
    {
        ...
        memolog::LogError("http3.lsn.start.fail", "ListenerStart failed", ef);
    ...
    running_ = true;
    memolog::LogInfo("gate.http3.started", "GateServer HTTP/3 listener started", {{"port", std::to_string(port_)}});
    return true;
#else
    (void) error;
    return false;
...
#endif
    memolog::LogInfo("gate.http3.stopped", "GateServer HTTP/3 listener stopped", {});
}
