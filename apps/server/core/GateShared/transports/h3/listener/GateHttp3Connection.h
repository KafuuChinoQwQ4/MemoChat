...
public:
    ...
    void SetConnIndex(int32_t idx)
    {
        conn_index_ = idx;
    }
    void SetHeadersComplete()
    {
        headers_received_ = true;
    }
    void SetStreamId(int64_t sid)
    {
        stream_id_ = sid;
    }
    ...
    std::string ResponseContentType() const
    {
    ...
    {
        return stream_open_.load();
    }

    // Methods for GateHttp3Listener to use
    std::string& GetRequestMethodRef()
    {
        return request_method_;
    }
    std::string& GetRequestPathRef()
    {
        return request_path_;
    }
    int64_t& GetContentLengthRef()
    {
