...
using Http3AuthHandler =
    bool (memochat::gate::services::auth::AuthService::*)(const memochat::gate::routing::GateRequest&,
                                                          memochat::gate::routing::GateResponse&);
...
void EnsureSharedRouteRegistry()
{
    ...
    {
        memochat::gate::modules::health::HealthRouteModule("GateServer-HTTP3").RegisterRoutes(SharedRouteRegistry());
        return true;
    }();
    (void) registered;
}
...
memochat::gate::routing::GateRequest BuildHttp3AuthRequest(const std::shared_ptr<GateHttp3Connection>& connection,
                                                           std::string_view route_path)
{
    memochat::gate::routing::GateRequest request;
    request.method = "POST";
    request.path = std::string(route_path);
    request.target = connection ? connection->GetRequestPath() : request.path;
    request.body = connection ? connection->GetRequestBody() : "";
    request.trace_id = connection ? connection->GetTraceId() : "";
    request.request_id = connection ? connection->GetRequestId() : "";
    request.headers = connection ? connection->GetRequestHeaders() : std::unordered_map<std::string, std::string>{};
    return request;
}

bool HandleHttp3AuthRoute(const std::shared_ptr<GateHttp3Connection>& connection,
                          std::string_view route_path,
                          Http3AuthHandler handler)
{
    if (!connection)
    {
        return false;
    ...
    const auto request = BuildHttp3AuthRequest(connection, route_path);
    memolog::TraceContext::SetTraceId(request.trace_id);
    memolog::TraceContext::SetRequestId(request.request_id);

    memochat::gate::routing::GateResponse response;
    const bool handled = (memochat::gate::services::auth::AuthService::Instance().*handler)(request, response);
    if (!handled)
    {
        return false;
    }
    ...
    return connection->SendResponse(response.status, response.body, content_type);
}
...
void RegisterRoutes(LogicSystem& logic)
{
    logic.RegPost("/get_varifycode",
                  ...
                  {
                      return HandleHttp3AuthRoute(connection,
                                                  "/get_varifycode",
                                                  &memochat::gate::services::auth::AuthService::HandleGetVarifyCode);
                  });
    ...
    logic.RegPost("/user_register",
                  [](std::shared_ptr<GateHttp3Connection> connection)
                  {
                      return HandleHttp3AuthRoute(connection,
                                                  "/user_register",
                                                  &memochat::gate::services::auth::AuthService::HandleUserRegister);
                  });
    ...
    logic.RegPost("/reset_pwd",
                  [](std::shared_ptr<GateHttp3Connection> connection)
                  {
                      return HandleHttp3AuthRoute(connection,
                                                  "/reset_pwd",
                                                  &memochat::gate::services::auth::AuthService::HandleResetPwd);
                  });
    ...
    logic.RegPost("/user_login",
                  [](std::shared_ptr<GateHttp3Connection> connection)
                  {
                      return HandleHttp3AuthRoute(connection,
                                                  "/user_login",
                                                  &memochat::gate::services::auth::AuthService::HandleUserLogin);
                  });
    ...
    logic.RegGet("/healthz",
                 [](std::shared_ptr<GateHttp3Connection> connection)
                 {
                     return DispatchSharedRoute(connection);
                 });
        ...
        [](std::shared_ptr<GateHttp3Connection> connection)
        {
            return GateHttp3JsonSupport::HandleJsonPost(
                ...
                [](const memochat::json::JsonValue& src_root, memochat::json::JsonValue& root, const std::string&)
                {
                    ...
                    auto result = Http2MediaSupport::HandleUploadMediaSimple(uid,
                                                                             token,
                                                                             media_type,
                                                                             file_name,
                                                                             mime,
            ...
            return GateHttp3JsonSupport::HandleJsonPost(
                ...
                [](const memochat::json::JsonValue& src_root, memochat::json::JsonValue& root, const std::string&)
                {
                    ...
                    if (moment_id > 0 && !MongoMgr::GetInstance()->InsertMomentContent(content))
                    {
                        ...
                        root["error"] = ErrorCodes::RPCFailed;
                        memolog::LogError("gate.http3.moments.publish",
            ...
            return GateHttp3JsonSupport::HandleJsonPost(
                ...
                [](const memochat::json::JsonValue& src_root, memochat::json::JsonValue& root, const std::string&)
                {
                    ...
                    PostgresMgr::GetInstance()
                        ->GetMomentsFeed(uid, last_moment_id, limit, author_uid, moments, has_more);
        ...
        [](std::shared_ptr<GateHttp3Connection> connection)
        {
            return GateHttp3JsonSupport::HandleJsonPost(
                ...
                [](const memochat::json::JsonValue& src_root, memochat::json::JsonValue& root, const std::string&)
                {
                    ...
                    bool has_more = false;
                    PostgresMgr::GetInstance()->GetMomentComments(moment_id,
                                                                  last_comment_id,
                                                                  limit,
                                                                  comments,
                                                                  has_more);
        ...
        [](std::shared_ptr<GateHttp3Connection> connection)
        {
            return GateHttp3JsonSupport::HandleJsonPost(
                ...
                [](const memochat::json::JsonValue& src_root, memochat::json::JsonValue& root, const std::string&)
                {
                    ...
                    const bool ok = like ? PostgresMgr::GetInstance()->AddMomentCommentLike(comment_id, uid)
