...
GetChatServerRsp StatusGrpcClient::GetChatServer(int uid)
{
    ...
    memolog::LogInfo("grpc.call",
                     "calling StatusService.GetChatServer",
                     {{"rpc", "StatusService.GetChatServer"},
                      {"uid", std::to_string(uid)},
                      {"peer_service", "StatusServer"},
                      {"module", "grpc"}});
    ...
    else
    {
        ...
        span.SetAttribute("rpc.grpc.status_code", std::to_string(status.error_code()));
        memolog::LogWarn("grpc.call.failed",
                         "StatusService.GetChatServer failed",
                         {{"rpc", "StatusService.GetChatServer"},
...
LoginRsp StatusGrpcClient::Login(int uid, std::string token)
{
    ...
    else
    {
        ...
        span.SetAttribute("rpc.grpc.status_code", std::to_string(status.error_code()));
        memolog::LogWarn("grpc.call.failed",
                         "StatusService.Login failed",
                         {{"rpc", "StatusService.Login"},
                          {"error", status.error_message()},
                          {"peer_service", "StatusServer"},
                          {"module", "grpc"},
                          {"error_type", "grpc"}});
