...
void AIHttpServiceRoutes::RegisterRoutes(LogicSystem& logic)
{
    ...
    logic.RegPost("/ai/chat",
                  [](std::shared_ptr<HttpConnection> connection)
                  {
                      ...
                      const int error_code =
                          json::glaze_safe_get<int>(result, "error", json::glaze_safe_get<int>(result, "code", 0));
                      if (error_code != 0)
                      {
    ...
    logic.RegPost(
        ...
        [](std::shared_ptr<HttpConnection> connection)
        {
            ...
            std::string model_name = json::glaze_safe_get<std::string>(src_root, "model_name", "");
            std::string deployment_preference =
            ...
            auto result = g_ai_client->Smart(uid,
                                             feature_type,
                                             content,
                                             target_lang,
                                             context_json,
    ...
    logic.RegGet("/ai/history",
                 [](std::shared_ptr<HttpConnection> connection)
                 {
                     ...
                     {
                         if (param.first == "uid")
                             uid = std::stoi(param.second);
                         else if (param.first == "session_id")
                             session_id = param.second;
    ...
    logic.RegGet("/ai/session/list",
                 [](std::shared_ptr<HttpConnection> connection)
                 {
                     ...
                     for (const auto& param : connection->_get_params)
                     {
                         if (param.first == "uid")
                             uid = std::stoi(param.second);
                         else if (param.first == "model_type")
                             model_type = param.second;
                         else if (param.first == "model_name")
                             model_name = param.second;
                     }
    ...
    logic.RegPost("/ai/session/delete",
                  [](std::shared_ptr<HttpConnection> connection)
                  {
    ...
    logic.RegPost("/ai/model/api/register",
                  [](std::shared_ptr<HttpConnection> connection)
                  {
                      ...
                      std::string provider_name =
                          json::glaze_safe_get<std::string>(src_root, "provider_name", "custom-api");
    ...
    logic.RegPost("/ai/model/api/delete",
                  [](std::shared_ptr<HttpConnection> connection)
                  {
    ...
    logic.RegPost("/ai/memory/delete",
                  [](std::shared_ptr<HttpConnection> connection)
                  {
    ...
    logic.RegPost("/ai/tasks",
                  [](std::shared_ptr<HttpConnection> connection)
                  {
                      ...
                      std::string metadata_json = ExtractMetadataJson(src_root);
                      auto result = g_ai_client->AgentTaskCreate(uid,
                                                                 title,
                                                                 content,
                                                                 session_id,
                                                                 model_type,
    ...
    logic.RegGet("/ai/tasks/detail",
                 [](std::shared_ptr<HttpConnection> connection)
                 {
                     ...
                     for (const auto& param : connection->_get_params)
                     {
                         if (param.first == "task_id")
                             task_id = param.second;
                     }
