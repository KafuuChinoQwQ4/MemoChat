...
void RegisterHttp2MomentsRoutes()
{
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::Error_Json, "invalid auth params")));
                return;
            }
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::RPCFailed, "add moment failed")));
                return;
            }
                ...
                if (memochat::json::glaze_is_array(items_val))
                {
                    ...
                    if (arr)
                    {
                        for (size_t idx = 0; idx < arr->size(); ++idx)
                        {
                            ...
                            MomentItemInfo item;
                            item.media_type = memochat::json::glaze_safe_get<std::string>(item_json,
                                                                                          "media_type",
                                                                                          std::string("text"));
                            item.media_key =
                                memochat::json::glaze_safe_get<std::string>(item_json, "media_key", std::string());
                            item.thumb_key =
                                memochat::json::glaze_safe_get<std::string>(item_json, "thumb_key", std::string());
                            item.content =
                                memochat::json::glaze_safe_get<std::string>(item_json, "content", std::string());
            ...
            if (moment_id > 0 && !MongoMgr::GetInstance()->InsertMomentContent(content))
            {
                ...
                resp.SetJsonBody(memochat::json::glaze_stringify(
                    MakeMomentsError(ErrorCodes::RPCFailed, "store moment content failed")));
                return;
            }
    ...
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            {
                resp.SetJsonBody(memochat::json::glaze_stringify(
                    MakeMomentsError(ErrorCodes::RPCFailed, "get moments feed failed")));
                return;
            }
    ...
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::Error_Json, "invalid moment_id")));
                return;
            }
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::RPCFailed, "moment not found")));
                return;
            }
            ...
            {
                resp.SetJsonBody(memochat::json::glaze_stringify(
                    MakeMomentsError(ErrorCodes::CallPermissionDenied, "permission denied")));
                return;
            }
    ...
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            bool ok = like ? PostgresMgr::GetInstance()->AddMomentLike(moment_id, uid)
                           : PostgresMgr::GetInstance()->RemoveMomentLike(moment_id, uid);
            if (!ok)
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::RPCFailed, "like operation failed")));
                return;
            }
            MomentInfo moment;
    ...
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            bool delete_mode =
                memochat::json::glaze_safe_get<bool>(root, "delete", false) || (comment_id > 0 && content_str.empty());
            ...
            if (delete_mode)
            {
                ...
                {
                    resp.SetJsonBody(memochat::json::glaze_stringify(
                        MakeMomentsError(ErrorCodes::Error_Json, "invalid comment_id")));
                    return;
                }
                bool ok = PostgresMgr::GetInstance()->DeleteMomentComment(comment_id, uid);
                if (!ok)
                {
                    memolog::LogWarn("gate.http2.moments.comment.delete",
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::Error_Json, "invalid params")));
                return;
            }
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::RPCFailed, "add comment failed")));
                return;
            }
            MomentInfo moment;
            PostgresMgr::GetInstance()->GetMomentById(moment_id, moment);
    ...
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::RPCFailed, "get comments failed")));
                return;
            }
    ...
    Http2Routes::RegisterHandler(
        ...
        [](const Http2Request& req, Http2Response& resp)
        {
            ...
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::Error_Json, "invalid comment_id")));
                return;
            }

            bool ok = like ? PostgresMgr::GetInstance()->AddMomentCommentLike(comment_id, uid)
                           : PostgresMgr::GetInstance()->RemoveMomentCommentLike(comment_id, uid);
            if (!ok)
            {
                resp.SetJsonBody(
                    memochat::json::glaze_stringify(MakeMomentsError(ErrorCodes::RPCFailed, "comment like failed")));
                return;
            }
