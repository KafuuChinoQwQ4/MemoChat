...
HTTP 登录返回的 `access_token` 是 HMAC-SHA256 签名的 JWT。签名密钥配置在各 Gate/Focused
gateway ini 的 `[AuthToken] JwtSecret`，生产必须通过环境变量覆盖：

```
MEMOCHAT_AUTHTOKEN_JWTSECRET
```

该密钥只属于服务端信任边界，不写入客户端配置、不返回给客户端、不放进网页端构建产物。生产启动会拒绝空值、
众所周知 dev 值 `memochat-dev-access-token-secret`，以及短于 32 字节的值。
