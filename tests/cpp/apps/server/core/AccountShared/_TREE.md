...
| `AuthServiceAlgorithmsConsumer.cpp` | 直接 import 认证服务 module，向测试暴露响应元数据/uid 守卫/传输选择 helper |
| `AuthServiceAlgorithmsTest.cpp` | 验证认证服务 module 导出的响应状态/内容类型、注册 uid 守卫与传输选择常量 |
| `CMakeLists.txt` | 注册该测试目标的 CMake 配置 |
| `GateAsyncSideEffectAlgorithmsConsumer.cpp` | 直接 import 异步副作用 module，向测试暴露配置守卫、topic/错误字面量 helper |
| `GateAsyncSideEffectAlgorithmsTest.cpp` | 验证异步副作用 module 导出的配置回退/覆盖判定与 Kafka/RabbitMQ 字面量 |
