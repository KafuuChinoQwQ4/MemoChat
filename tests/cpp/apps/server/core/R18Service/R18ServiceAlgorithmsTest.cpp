...
TEST(R18ServiceAlgorithmsTest, ExposesStableResponseMetadata)
{
    ...
    EXPECT_EQ(ServiceUnavailableHttpStatus(), 503);
    EXPECT_EQ(MaxConcurrentImageRequests(), 4);
    EXPECT_TRUE(ShouldAdmitImageFetch(3));
    EXPECT_FALSE(ShouldAdmitImageFetch(4));
    EXPECT_STREQ(ImageBusyMessage(), "image service busy; retry shortly");
