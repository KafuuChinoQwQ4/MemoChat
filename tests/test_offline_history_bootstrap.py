...
class OfflineHistoryBootstrapTests(unittest.TestCase):
    ...
    def test_private_bootstrap_history_does_not_clear_current_pending_state(self):
        ...
        compact = compact_ws(body)
        self.assertIn("const bool isPendingRequest = peerUid == _private_history_state.pendingPeerUid;", body)
        self.assertIn("if (isPendingRequest) { setPrivateHistoryLoading(false); }", compact)
        self.assertIn("if (isPendingRequest) { _private_history_state.pendingBeforeTs = 0;", compact)
    ...
    def test_select_dialog_by_group_uid_falls_back_without_group_list_row(self):
        source = APP_CONTROLLER_DIALOG_SELECTION.read_text(encoding="utf-8")
        body = extract_function(source, "void AppController::selectDialogByUid")
        self.assertIn("ConversationSyncService::groupIdForDialogUid(_group_state.dialogUidMap, uid)", body)
    ...
    def test_select_group_index_derives_group_id_when_uid_map_was_refreshed(self):
        ...
        self.assertIn("ConversationSyncService::groupIdForDialogUid(_group_state.dialogUidMap, pseudoUid)", body)
    ...
    def test_group_history_request_is_not_blocked_by_private_history_loading(self):
        ...
        self.assertNotIn("if (_private_history_loading) {\n        return;\n    }", body)
        self.assertIn("_loading_state.groupHistoryLoading", body)
