...
GATE_H2_HANDLERS = SERVER_CORE / "GateServer" / "transports" / "h2" / "handlers"
...
class MediaUploadContractTest(unittest.TestCase):
    def test_direct_gate_and_h1_raise_beast_request_body_limit_for_media_chunks(self):
        ...
        h1 = read_text(SERVER_CORE / "GateServer" / "transports" / "h1" / "legacy_standalone" / "H1Connection.cpp")
    ...
    def test_h1_media_routes_are_migrated_to_shared_support(self):
        source = read_text(
            SERVER_CORE / "GateServer" / "transports" / "h1" / "legacy_standalone" / "H1MediaService.cpp"
        )
    ...
    def test_shared_media_support_has_raw_binary_chunk_helper(self):
        header = read_text(SERVER_CORE / "GateServer" / "core" / "support" / "Http2MediaSupport.h")
        source = read_text(SERVER_CORE / "GateServer" / "core" / "support" / "Http2MediaSupport.cpp")
    ...
    def test_direct_gate_media_service_persists_upload_session_as_object(self):
        source = read_text(SERVER_CORE / "GateServer" / "services" / "media" / "MediaService.cpp")
        save_func = re.search(
            r"bool SaveJsonFileLocal\(const std::filesystem::path& path, const memochat::json::JsonValue& root\)"
            r"(?P<body>.*?)\n\}",
            source,
    ...
    def test_e4_local_file_download_contract_uses_gate_response_file_body_kind(self):
        response_header = strip_comments(read_text(SERVER_CORE / "GateServer" / "routing" / "GateResponse.h"))
        h1_adapter_source = strip_comments(
            read_text(SERVER_CORE / "GateServer" / "adapters" / "h1" / "H1RouteAdapter.cpp")
        )
        ...
        media_source = read_text(SERVER_CORE / "GateServer" / "services" / "media" / "MediaService.cpp")
