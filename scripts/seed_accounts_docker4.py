#!/usr/bin/env python3
"""Seed accounts into Docker PostgreSQL with proper search_path=memo."""
import json
import psycopg
...
def main():
    ...
    print(f"[seed] Existing perf_test accounts: {before}")
    ...
    print(f"[seed] perf_test accounts after: {after} (net new: {after - before})")
    ...
    for r in rows[:3]:
        ...
        if row:
            ...
            print(f"  uid={r['uid']} decoded={repr(decoded)} [{ok}]")
        else:
            print(f"  uid={r['uid']} NOT FOUND")
